...
C                 end of iterations if step change less than factor 2
            ...
            STEPB4 = STEP
c
c   Good chance parallel execution will speed this up
c
...
C           take two steps
         X(I) = XTF + D
c
c   may be a good place for parallelization
c
         CALL MNINEX(X)
         NPARX = NPAR
         CALL FCN(NPARX,GIN,FS1,U,4,FUTIL)
...
c
c     another place for parallelization
c
      DO 200 I= 1, NPAR
         DO 180 J= 1, I-1
            XTI = X(I)
...
C                                       iterate reducing step size
      DO 50 ICYC= 1, NCYC
c
c    a suggestion for parallelization
c
         X(I) = XTF + D
         CALL MNINEX(X)
         CALL FCN(NPARX,GIN,FS1,U,4,FUTIL)
...
C---------------------------------------
      DO I=1,NXYPT
       WRITE(79,79)WORD7(1),XPT(I),YPT(I)
   79  FORMAT(2X,F10.3,2X,F15.7,2X,F15.7)
      ENDDO
      XXXX=9999.
      WRITE(79,79)XXXX,XXXX,XXXX
C---------------------------------------
